...
const POS = () => {
  ...
  const addToCart = (product, customToppings = []) => {
    const productWithToppings = {
      ...product,
      quantity: 1,
      toppings: customToppings,
      totalPrice: product.price + customToppings.reduce((sum, topping) => sum + topping.price, 0),
      uniqueId: Date.now() + Math.random()
    };

    setCart([...cart, productWithToppings]);
  };
      ...
      <div className="max-w-7xl mx-auto">
              ...
              {loading ? (
                ...
                filteredProducts.map(product => (
                  <div
                    ...
                    onClick={() => product.hasCustomization ? openCustomizationModal(product) : addToCart(product)}
                  >
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="bg-gradient-to-r from-amber-500 to-orange-600 text-white p-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity">
                          {product.hasCustomization ? <FiZap className="h-4 w-4" /> : <FiPlus className="h-4 w-4" />}
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              {cart.length === 0 ? (
                ...
                <>
                  <div className="space-y-3 mb-6 max-h-64 overflow-y-auto">
                    {cart.map(item => (
                      <div key={item.uniqueId} className="bg-gray-700/50 rounded-xl p-3 border border-gray-600/50">
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-medium text-white text-sm">{item.name}</span>
                          <button
                            onClick={() => updateQuantity(item.uniqueId, 0)}
                        ...
                        {item.toppings && item.toppings.length > 0 && (
                          <p className="text-xs text-gray-400 mb-2">+ {item.toppings.map(t => t.name).join(', ')}</p>
                        )}
                        <div className="flex items-center justify-between">
        ...
        {showModal && selectedProduct && (
          <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-800/95 backdrop-blur-sm rounded-3xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-gray-700/50">
              {/* Header del modal */}
              <div className="flex items-center justify-between p-6 border-b border-gray-700/50">
                <div className="flex items-center gap-4">
                  <div className="text-4xl">
                    {selectedProduct.category === 'waffles' && '🧇'}
                    {selectedProduct.category === 'helados' && '🍦'}
                    {selectedProduct.category === 'bebidas' && '🥤'}
                    {selectedProduct.category === 'personalizados' && '⭐'}
                  </div>
                  <div>
                ...
                <button
                  ...
                  className="p-2 hover:bg-gray-700/50 rounded-full transition-all duration-200 text-gray-400 hover:text-white"
                >
              ...
              {/* Contenido del modal */}
              <div className="p-6">
                <h3 className="text-xl font-bold text-white mb-6">Personaliza tu {selectedProduct.name}</h3>

                {/* Categorías de toppings */}
                      ...
                      <div key={category.id}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {categoryToppings.map((topping) => {
                            ...
                            const isAvailable = topping.stock > 0;
                            return (
                              <button
                                key={topping.id}
                                ...
                                className={`p-4 rounded-2xl border-2 transition-all duration-200 text-left ${
                                  !isAvailable
                                    ...
                                    : 'border-gray-600 bg-gray-700/50 hover:border-gray-500 hover:shadow-md'
                                }`}
                              >
                                <div className="flex items-center justify-between">
                                  <div className="flex-1">
                                    <h5 className={`font-semibold ${!isAvailable ? 'text-gray-500' : 'text-white'}`}>
                                      {topping.name}
                                    </h5>
                                    <p className={`font-bold ${!isAvailable ? 'text-gray-600' : 'text-amber-400'}`}>
                                      {formatCurrency(topping.price)}
                                    </p>
                                    <span className={`inline-block mt-1 px-2 py-1 text-xs font-medium rounded-full ${
                                      isAvailable ? 'bg-green-900/50 text-green-300' : 'bg-red-900/50 text-red-300'
                                    }`}>
                                      {topping.stock} disponibles
                                    </span>
                                  </div>
              ...
              {/* Footer del modal */}
              <div className="flex items-center justify-between p-6 border-t border-gray-700/50 bg-gray-800/50 rounded-b-3xl">
                <div>
                  <p className="text-sm text-gray-400">Total con personalizaciones:</p>
                  <p className="text-2xl font-bold text-amber-400">{formatCurrency(calculateModalTotal())}</p>
                  {selectedToppings.length > 0 && (
                    <p className="text-xs text-gray-500 mt-1">
                      Incluye: {selectedToppings.map(t => t.name).join(', ')}
                    </p>
                  )}
                </div>
                <div className="flex gap-3">
                  <button
                    onClick={() => setShowModal(false)}
                    className="px-6 py-3 bg-gray-700 text-gray-300 font-semibold rounded-xl border border-gray-600 hover:bg-gray-600 hover:text-white transition-all duration-200"
                  >
                    Cancelar
                  </button>
                  <button
                    onClick={addToCartFromModal}
                    className="px-8 py-3 bg-gradient-to-r from-amber-500 to-orange-600 text-white font-bold rounded-xl transition-all duration-200 shadow-lg hover:shadow-xl hover:from-amber-600 hover:to-orange-700 flex items-center gap-2"
                  >
